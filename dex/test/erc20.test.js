...
contract("ERC20 token test", (accounts) => {
  ...
  // contractにasyncは指定できないのでbeforeで書く。
  before(async () => {
  ...
  describe("Supply and balance test", () => {
    it("Should have the correct total supply", async () => {
      ...
      // BNにはto.be.equalではなく、to.be.bignumber.equalとする。
  ...
  describe("transfer() test", () => {
    it("Should revert when transfer amount > balance", async () => {
      ...
      // 残高以上を送ろうとして失敗することをテスト。
      // https://www.npmjs.com/package/truffle-assertions/v/0.6.3#truffleassertrevertsasyncfn-reason-message
    ...
    it("Should update balances accordingly", async () => {
      ...
      // https://ethereum.stackexchange.com/questions/60720/how-to-fix-no-events-were-emitted
      expect(ownerBalance).to.be.bignumber.equal(totalSupply.sub(thousand)); // 引き算
